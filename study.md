...
```md
Brackets [] denote the optional parts of a command.
Braces such as () and {} as well as pipes || indicate that you must choose one alternative.
```
...
```md
Ellipsis (...) indicate that the preceding element can be repeated.
```
...
# I'm assuming we won't need the $ in front of it since it indicates the prompt.
```
